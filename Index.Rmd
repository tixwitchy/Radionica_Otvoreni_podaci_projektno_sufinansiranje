...
grafikon <- tabela %>%
  ...
  filter(`ORGAN KOJI RASPISUJE KONKURS/OPŠTINA`=="Ada")%>%
...
<section>
<a href="https://tixwitchy.shinyapps.io/Media_Funding_in_Serbia/">
<img class="center" height="500" src="Masterteza.png"> </a>
</section>
...
- Obuhvatio je **`r n_distinct(tabela[,1])`** davaoca sredstava: Pokrajinski sekretarijat, Ministarstvo kulture i informisanja i **`r n_distinct(tabela[,1])-2`** lokalne samouprave;
...
DT :: datatable (tabelaAda,rownames = FALSE, options = list(
...
DT :: datatable (tabelaAdafin,rownames = FALSE, options = list(
  bPaginate = TRUE, scrollX=T, lengthMenu=c(5,8), info= FALSE,initComplete = DT::JS(
    "function(settings, json) {",
...
bci <-  ggplot (data = grafikon, aes (x = reorder(`PODNOSILAC PROJEKTA`, -`SREDSTVA U EVRIMA`), y = `SREDSTVA U EVRIMA`,label = `Info`))+
        ...
        ggtitle (paste( "Pet podnosilaca\n koji su dobili najviše sredstava\n od organa/opštine", barchart[1,1],"\n"))+
